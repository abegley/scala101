...
object exN extends App {
  ...
  def query(orders: Seq[Order]):Map[String, Float] = {
    val adultOrders = orders.filter(_.customer.age >= 18)
    val ordersByPostcode = adultOrders.groupBy(_.billingAddress.postCode)
  ...
  def createOrder(i:Int):Order = {
    ...
    val items = List( Item("ZI000001", i),
                      Item("ZI000001", 0.55f),
                      Item("ZI000001", 75.45f))
